...
async function LoginEmployee(data: SigninInterface) {
...
async function LoginDentist(data: SigninInterface) {
  const requestOptions = {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(data),
  ...
  let res = await fetch(`${apiUrl}/login_dentist`, requestOptions)
    .then((response) => response.json())
    .then((res) => {
      if (res.data) {
        console.log(res.data);
